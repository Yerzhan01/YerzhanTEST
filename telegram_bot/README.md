...
User → Telegram → Worker (/webhook) ──► kie.ai (createTask + callBackUrl)
                          ↓
                    KV (TASKS)
                          ↑     ┌── путь A: kie.ai callback (мгновенно)
                          │     │
                          └─────┤
                                │
                                └── путь B: кнопка "🔄 Проверить" (вручную)
                          ↓
                  Telegram → User
```
...
Доставка результата по двум путям: kie.ai сам шлёт callback на воркер
(основной путь), либо пользователь жмёт «🔄 Проверить» под сообщением
с задачей. Если хочешь авто-fallback на случай потерянного callback —
добавь `[triggers].crons` в `wrangler.toml` и `scheduled()` в `index.ts`
(было раньше; убрано чтобы не жечь cron-инвокации впустую).
...
2. **Кнопка 🔄 Проверить** в чате → пинг по запросу пользователя
...
| `wrangler.toml` | Конфиг воркера, KV, observability |
